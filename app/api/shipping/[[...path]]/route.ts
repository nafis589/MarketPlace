...
async function proxyShipping(request: NextRequest, segments: string[] | undefined) {
  const suffix = buildBackendPath(segments);
  const url = `${API_URL}/api/store/shipping${suffix}`;
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  return proxyShipping(request, path);
}
